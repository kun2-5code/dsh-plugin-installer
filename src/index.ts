/**
 * dsh-plugin-installer 主插件：一个可直接运行的示例，演示 dsh 插件最常用的四种形态——
 ...
 *
 * 配置来源：settings 服务存在时，把它注册为命名空间 `dsh-plugin-installer`
...
export function apply(ctx: Context, config: Config): void {
  ...
  installSettingsSection(ctx, settingsNamespace('dsh-plugin-installer'), Config, config, {
